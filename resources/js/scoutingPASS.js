...
function addElement(table, idx, data) {
    ...
    if (data.hasOwnProperty('required')) {
        if (data.required.toLowerCase() == 'true') {
            if (requiredFields[table.parentElement.id] == undefined) {
                requiredFields[table.parentElement.id] = []
            }
            requiredFields[table.parentElement.id].push(data.code)
...
function validateData() {
    var ret = true;
    var errStr = "";
    for (const [tableName, tableRequiredFields] of Object.entries(requiredFields)) {
        if (document.getElementById(tableName).style.display == "table") {
            for (rf of tableRequiredFields) {
...
function swipePage(increment, validate = true) {
    if ((increment < 0 || !validate) || qr_regenerate() == true) {
        previousSlide = slide
...
function toFoul() {
    increment = 4 - slide
    swipePage(increment, false)
}
function returnPage() {
    increment = previousSlide - slide
    swipePage(increment, false)
